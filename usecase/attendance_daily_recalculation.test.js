/**
 * THE DAILY 06:55 AUTOMATIC ATTENDANCE RECALCULATION, and the 07:00 Missing
...
const OVERNIGHT = 8; // 22:00-07:00, cutoff 09:00 next morning - still OPEN at 06:55
...
describe("A/B. the schedules, in Asia/Kolkata, in the required order", () => {
  ...
  it("A. the daily recalculation is registered at 06:55", () => {
  ...
  it("both run in Asia/Kolkata - the zone every in-process cron is scheduled in", async () => {
    ...
    try {
      ...
      service.register("attendance_daily_recalculation", "55 6 * * *", async () => {});
...
describe("C. the rolling window is today-3 through yesterday", () => {
  ...
  it("the 06:55 run on 2026-09-24 asks recalculateBulk for exactly that range, as a SYSTEM run", async () => {
    const calls = [];
    const fake = {
    ...
    const { daily, audit } = job(fake, ist("2026-09-24", 6, 55));
    ...
    assert.deepEqual(
      ...
      [{ from_date: "2026-09-21", to_date: "2026-09-23", actor_employee_id: null, record_run: false, now: ist("2026-09-24", 6, 55) }]
    );
  ...
  it("writes NO attendance_recalculation_run row - it is not a manual run", async () => {
    ...
    const { daily } = job(buildCalculation(c.repo), ist("2026-09-24", 6, 55));
    await daily.run();
...
describe("D/E. the closed-date guard stays authoritative", () => {
  it("D. yesterday still open under an overnight cutoff (09:00) is skipped at 06:55; E. closed dates are stored", async () => {
    const c = company({
    ...
    const next = job(buildCalculation(c.repo), ist("2026-09-25", 6, 55));
...
describe("F. payroll-locked dates remain unchanged", () => {
  it("a locked September is untouched; the open October date beside it is still stored", async () => {
    ...
    const { daily, audit } = job(buildCalculation(c.repo), ist("2026-10-02", 6, 55));
    const summary = await daily.run();
...
describe("G. a late punch for 22-Sep is repaired by the next morning's run", () => {
  it("22-Sep stored with 3 punches on the 23rd; a 4th arrives; the 24th's run (21..23) stores 4", async () => {
    ...
    await job(calculation, ist("2026-09-23", 6, 55)).daily.run(); // window 20..22
    ...
    await job(calculation, ist("2026-09-24", 6, 55)).daily.run(); // window 21..23
...
describe("H. the job never overlaps itself", () => {
  it("a second invocation while the first is running is a logged no-op; the next one after it runs", async () => {
    ...
    const { daily, lines, audit } = job(fake, ist("2026-09-24", 6, 55));
...
describe("I. a failure is logged and never stops the 07:00 Telegram", () => {
  it("a throwing recalculation resolves to a FAILED summary, is logged and audited, and does not throw", async () => {
    ...
    const { daily, audit, errors } = job(fake, ist("2026-09-24", 6, 55));
    const summary = await daily.run();
  ...
  it("the two are separate cron registrations: a failing 06:55 job does not prevent the 07:00 one from firing", async () => {
    const CronService = require("../services/cron_service");
    const nodeCron = require("node-cron");
    const original = nodeCron.schedule;
    ...
    try {
      ...
      service.register("attendance_daily_recalculation", "55 6 * * *", async () => { throw new Error("boom"); });
    ...
    assert.equal(cronErrors.length, 1, "the 06:55 failure was logged by the wrapper, not thrown");
  });
});
