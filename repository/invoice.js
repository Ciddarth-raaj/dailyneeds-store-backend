...
class InvoiceRepository {
  ...
  createInvoiceWithConnection(connection, invoice) {
    return new Promise((resolve, reject) => {
      ...
      if (invoiceId) {
        ...
        connection.query(
          `INSERT INTO invoice (invoice_id, supplier_id, created_at, updated_at) VALUES (?, ?, NOW(), NOW())`,
          [invoiceId, invoice.supplier_id || null],
          (err, res) => {
            if (err) {
        ...
        connection.query(
          `INSERT INTO invoice (invoice_id, supplier_id, created_at, updated_at) VALUES (?, ?, NOW(), NOW())`,
          [autoGeneratedId, invoice.supplier_id || null],
          (err, res) => {
            if (err) {
              logger.Log({
                level: logger.LEVEL.ERROR,
  ...
  updateInvoiceWithConnection(connection, invoiceId, invoiceData) {
    return new Promise((resolve, reject) => {
      ...
      if (invoiceData.supplier_id !== undefined) {
        updateFields.push('supplier_id = ?');
        updateValues.push(invoiceData.supplier_id);
      }
      ...
      connection.query(
        `UPDATE invoice SET ${updateFields.join(', ')} WHERE invoice_id = ?`,
        updateValues,
        (err, results) => {
          if (err) {
            logger.Log({
