...
const TABLE = "remarks_master";

class RemarksMasterRepository {
  constructor(db) {
    this.db = db;
  }
        ...
        (err, rows) => {
          if (err) {
            logger.Log({
              ...
              component: "REPOSITORY.REMARKS_MASTER",
              code: "REPOSITORY.REMARKS_MASTER.GET_ALL",
        ...
        (err, res) => {
          if (err) {
            logger.Log({
              level: logger.LEVEL.ERROR,
              component: "REPOSITORY.REMARKS_MASTER",
              code: "REPOSITORY.REMARKS_MASTER.CREATE",
              description: err.toString(),
              category: "",
              ref: {}
            });
        ...
        (err, res) => {
          if (err) {
            logger.Log({
              level: logger.LEVEL.ERROR,
              component: "REPOSITORY.REMARKS_MASTER",
              code: "REPOSITORY.REMARKS_MASTER.UPDATE",
              description: err.toString(),
              category: "",
              ref: {}
            });
        ...
        (err, res) => {
          if (err) {
            logger.Log({
              level: logger.LEVEL.ERROR,
              component: "REPOSITORY.REMARKS_MASTER",
              code: "REPOSITORY.REMARKS_MASTER.DELETE",
              description: err.toString(),
              category: "",
              ref: {}
            });
...
module.exports = (db) => {
  return new RemarksMasterRepository(db);
};
