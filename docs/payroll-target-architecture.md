...
There is no upside to renumbering. The only argument for it — "the PK is a
foreign system's business key" — is fully answered by decoupling instead:

```
employee_id          INT PK        keep every existing value. Stops being
                                   Digisme's code and becomes an opaque
                                   local surrogate key.
employee_code        VARCHAR(20)   NEW. UNIQUE. The human-facing code shown
                     UNIQUE        in the UI and on payslips. Backfilled for
                                   existing staff with their current
                                   employee_id as a string, so nothing that
                                   people read changes on migration day.
digisme_employee_code VARCHAR(20)  NEW. Nullable. Migration/audit reference
                     INDEX         only. Never joined on, never displayed
                                   after cutover. Kept so a dispute about a
                                   pre-cutover period can be traced back.
```

The column is already `AUTO_INCREMENT`; today rows arrive with the id
supplied, so the counter has never been used. Before cutover, set it above
the Digisme range with a gap that is obvious to the eye:

```sql
ALTER TABLE new_employee AUTO_INCREMENT = 100000;
```

New hires then get a locally allocated `employee_id`, and any id ≥ 100000 is
recognisably ours. `employee_code` for a new hire is generated by the app
(for example `DN100001`) and is free to follow whatever convention HR wants
...
edited. `digisme_employee_code` is the only trace, and it is inert.
...
| --- | --- | --- |
| `new_employee` | `employee_id` (match key only), `digisme_employee_code` | `employee_name`, `designation_id`, `department_id`, `store_id`, `shift_code`, `status`, `resignation_date`, `gender`, `marital_status`, `primary_contact_number` |
...
4. Delete `syncDigismeEmployees`, `_fetchDigismeEmployees`,
   ...
   `digisme_employee_code` and `digisme_sync_diff` — they are history.
...
| --- | --- | --- |
| Employee code | **E\*** | `employee_id` becomes an opaque key; add `employee_code VARCHAR(20) UNIQUE` as the display code (§1.1) |
...
payroll_payslip
  ...
  employee_code VARCHAR(20), employee_name VARCHAR(100),
...
| --- | --- | --- |
| Employee IDs | `new_employee.employee_id` | retained as-is (§1.1); no migration |
...
| Table | Change |
| --- | --- |
| `new_employee` | `employee_code` UNIQUE, `digisme_employee_code`, `employment_type`, `probation_status`, `probation_end_date`, `confirmation_date`, `reporting_manager_id` (self-FK), `default_shift_id` (FK), `weekly_off_rule_id`, `last_working_date`, `exit_type`, `exit_reason`, `rehire_eligible`, `pf_applicable`, `esi_applicable`, `pf_joining_date`. Type changes: `date_of_joining` → `DATE`, `gender` → ENUM, `employee_name` → `VARCHAR(100)`. Add the missing FKs to `designation`, `department`, `outlets`. `AUTO_INCREMENT = 100000`. Drop `shift_code` and `salary` after Stage 2. |
...
| **Employee id renumbering** | critical if attempted | do not attempt (§1.1). ~50 actor columns, only 12 constrained; a missed column silently repoints history. |
...
7. Extend `new_employee` with the fields in §3; migrate `date_of_joining`
   to `DATE`; add the missing FKs; set `AUTO_INCREMENT = 100000`; backfill
   `employee_code` and `digisme_employee_code`.
