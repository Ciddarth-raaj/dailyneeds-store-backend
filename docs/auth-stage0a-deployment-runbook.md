...
| 4 | Deployment trigger fully mapped (webhooks, runners, server hooks) | **NOT YET VERIFIED — host check ready** | | | Repository side complete (readiness §5.11: trigger, target, cwd, exact sequence, failure-stops-before-reload confirmed, db-migrate env-resolution rule, pending set = four Stage 0A). Run `scripts/auth/gate4-12-host-check.sh` (read-only) and confirm GitHub Webhooks/Runners pages. |
...
| 12 | trust proxy / forwarded-header spoofing, verified against real nginx | **NOT YET VERIFIED — host check ready** | | | Code-level PASS. Deployed prod uses `trust proxy true` (safe only while 8080 is not public and nginx overwrites headers); Stage 0A → loopback. Same host script + the external probe in readiness §5.11. |
