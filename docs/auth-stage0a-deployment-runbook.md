...
| 19A | Break-glass Telegram alert verified **in staging** | **NOT YET VERIFIED — FAILED on the first Lightsail run, fixed, rerun pending** | Administrator (Lightsail) / Claude (fix) | 07-09-2026 | First run: the bot reached Telegram but the alert was rejected, `400 Bad Request: can't parse entities` — `services/telegram` forced Markdown and the alert body contained the literal `user_id` (and the username `stage0a_breakglass`), which legacy Markdown reads as an unclosed italic entity. Fixed: security alerts are plain text with no parse mode, dynamic fields collapsed to one line; every other Telegram consumer keeps Markdown. Regression test `services/telegram.security_alerts.test.js` (9 cases) reproduces the exact rejection. Rerun the harness and confirm the plain-text alert arrives from `@DailyNeedsBot`. |
...
| Gate 19A rerun after the plain-text fix: `git pull --ff-only` then `MYSQL_BIN_DIR="$HOME/mysql84/bin" scripts/auth/staging-rehearsal.sh dnds_rehearsal`; confirm the `🚨 BREAK-GLASS LOGIN` message for `stage0a_breakglass` arrived from `@DailyNeedsBot` | Administrator | next | |
