...
| 4 | Deployment trigger fully mapped (webhooks, runners, server hooks) | **PASS except Webhooks verification pending repository-admin access** | Claude (repo + Actions API) / Administrator (host) / Owner (Webhooks) | 07-09-2026 | Trigger, target, sequence, failure semantics, env resolution, pending set, host facts, GitHub-hosted runners (Actions API) — all evidenced (readiness §5.11–5.12). Stale `deploy.yml` on `master` = accepted exception (inert for pushes; delete when convenient). Webhooks: the API access used here is refused on the hooks path (403 via proxy, both repos) and the administrator's account lacks repository settings; only the repository owner can confirm. Not blocking the remaining gates. |
| 5 | End-to-end restore rehearsal, snapshot ≤24 h | **PASS — fresh run 07-09-2026 at `c8d4c7b`, stamp `20260907-010839`, `GATE 5 REHEARSAL COMPLETE in 301s`** (first PASS 06-09-2026, below) | Administrator (Lightsail) / Claude (tooling) | 07-09-2026 | `gate5-rehearsal.sh --skip-backup` at `43d3d4b` on the `20260906-174727` artefacts: manifest reconstructed after independent re-verification; restore of `dnds_prod` (144 tables, 723.6 MB, dump 22 s) into `dnds_rehearsal` in **143 s**; 144 tables checked, **0 mismatches**; integrity checks OK; exactly four Stage 0A migrations pending; up / second up no-op / down×4 / up all clean; `GATE 5 REHEARSAL COMPLETE in 184 s`. Snapshot must be re-taken ≤24 h before deployment night (step 2 only). `dnds_rehearsal` kept in post-Stage-0A state for gates 13/14 and staging. |
...
| 9 | Frontend + backend staging login (normal, failed, logout, navigation) | **PASS (backend, on Lightsail against `dnds_rehearsal`)** | Administrator (Lightsail) / Claude (harness) | 07-09-2026 | `staging-rehearsal.sh` at `da4445e`: outlet default login flagged `weak_at_login`, admin strong login clean, wrong password 400, inactive employee refused, v2 token accepted on a protected route (readiness §5.12). Frontend-in-browser navigation against the staging instance remains a manual step. |
| 10 | Legacy-token staging transition, including ID-collision fixture | **PASS (on Lightsail against `dnds_rehearsal`)** | Administrator (Lightsail) / Claude (harness) | 07-09-2026 | Legacy tokens minted in the deployed shape with the current key: same user_id, employee_id cross-check 403, inactive refused on both shapes, never resolves to a system account, v2 `sub` + `kid` (readiness §5.12). |
...
| 17A | External JWT key loading proven **in staging** using the CURRENT key | **PASS (on Lightsail)** | Administrator (Lightsail) / Claude (script) | 07-09-2026 | `jwt-keys-setup.sh`: current pair copied to `~/.stage0a/jwt` (600), sign/verify proven both ways, staging instance ran on the external key with no tracked-key fallback. Production key untouched, not rotated. |
| 18A | Break-glass login verified **in staging** | **PASS (on Lightsail against `dnds_rehearsal`, 14 checks)** | Administrator (Lightsail) / Claude (harness) | 07-09-2026 | `stage0a_breakglass` on the scratch copy: NULL employee, system flag, scrypt, login 200 with `sys=true`, every employee/reset/unlock flow refused, row unchanged. Deployment B forced-change posture 6/6 on the second instance. |
| 19A | Break-glass Telegram alert verified **in staging** | **PASS (on Lightsail, plain-text alert received)** | Administrator (Lightsail + Telegram) / Claude (fix) | 07-09-2026 | First run at `da4445e` failed (`400 Bad Request: can't parse entities`: forced Markdown + `user_id`/`stage0a_breakglass` underscores). Fixed at `ffe6a4c` (plain-text security alerts, `parseMode: null`, one-line dynamic fields, regression test `services/telegram.security_alerts.test.js`). Rerun at `7a77b52`: 37/37 + 6/6, `STAGING REHEARSAL: ALL CHECKS PASSED`; the administrator personally received BOTH the `🚨 BREAK-GLASS LOGIN` alert and the failed-attempt alert in DN Accounts from `@DailyNeedsBot`. |
| 20 | All required tests green with named case mapping | **PASS (re-run at `ffe6a4c`: 588 tests / 587 pass / 0 fail / 1 unrelated skip)** | Claude / `node --test` | 07-09-2026 | `IS_TEST=true node --test`: **407 tests / 406 pass / 0 fail / 1 skip** (the skip is the unrelated `priceCheckerConflicts` golden test). New: `usecase/passwordReset.stage0a.test.js` 27, adapted upstream `usecase/passwordReset.test.js` 19, `routes/user.protection.test.js` +5. Mapping in readiness §0.4–0.8, §0.12. |
...
| 23 | Legacy token shape personally confirmed — decoded **locally** | **PASS** | Administrator (own PC, token never left it) | 07-09-2026 | Real current production token: header `alg=RS256 typ=JWT`; payload fields `id,store_id,designation_id,employee_id,user_type,iat,exp`; `has sub: False`, `has auth_ver: False`, `has kid: False`. Exactly the legacy shape Stage 0A resolves through `id` + `employee_id` only (`middlewares/auth.js`); the display claims (`name`, `designation`, `employee_image`) are absent from this token and are not used by resolution. |
...
### 2.1 Final review, 07-09-2026 — GO for Deployment A, with two explicitly accepted exceptions
...
- [ ] `TELEGRAM_BOT_TOKEN` already in the server `.env` (the Stage 0A code has no fallback); `JWT_PRIVATE_KEY_PATH`, `JWT_PUBLIC_KEYS`, `JWT_ACTIVE_KID` ready in `~/.stage0a/jwt.env` to be appended at step 8 (`JWT_LEGACY_KID` is optional: it defaults to `legacy`, the same value)
- [ ] Read-only pre-flight run the same day and ended `PRE-FLIGHT: ALL CHECKS PASS`: from `~/stage0a-rehearsal`, `scripts/auth/deployment-a-preflight.sh <fresh gate 5 stamp>` (checks the key env, token presence, deploy clone at `9d92884`, live `migrations` = 234 with none of the four Stage 0A ones, no Deployment-B flag, deploy targets, fresh artefacts + checksums; prints no secret)
...
4. **Code rollback first. Database restore only if required.**
   ...
   **Exact rollback commands (prepared 07-09-2026; run only when a trigger above fires):**

   *R1 — backend code, on the host, seconds (old code ignores the new columns and tables; the JWT lines in `.env` may stay, the old code does not read them):*
   ```bash
   cd ~/dailyneeds-store-backend && git fetch origin && git checkout --detach 9d92884 && npm i && pm2 reload 0 && pm2 logs 0 --lines 20 --nostream
   ```
   Then make the branch agree so the next workflow run does not redeploy Stage 0A: on a PC, `git revert -m 1 <merge commit sha>` on `main-autodeploy` and push (the workflow's `git checkout main-autodeploy && git pull` then lands on the revert). Never force-push.

   *R2 — frontend:* `git revert -m 1 <frontend merge commit sha>` on the frontend `main-autodeploy` and push; the workflow rebuilds and rsyncs the previous UI. The old UI logs in through the query string, which the reverted backend accepts.

   *R3 — schema, only under one of the four conditions above, from the rehearsal checkout with the deploy clone's config (each command reverts ONE migration; run it four times, newest first):*
   ```bash
   cd ~/stage0a-rehearsal/migrations/mysql && for i in 1 2 3 4; do NODE_PATH=~/stage0a-rehearsal/node_modules db-migrate down --config ~/dailyneeds-store-backend/migrations/mysql/database.json -e <env from gate 4/C> || break; done
   ```
   Down is exact (gate 22) and loses only Stage 0A data: `user_auth_log`, `user_password_reset`, `auth_metric`, the new `user` columns (including any scrypt hash — see the Telegram-reset note above) and the new permission keys.

   *R4 — data, last resort:* restore the RDS snapshot taken at step 5 through the AWS console (new instance, repoint `config.json`/`database.json`, reload), or restore the auth tables in place from the fresh gate 5 auth dump (`~/db-backups/dnds_prod-auth-<stamp>.sql.gz`, `--add-drop-table`, verified by checksum) — readiness §5.6.
...
| Gate 4: confirm Settings → Webhooks is empty on both repositories (screenshot or "empty") — needs repository-admin access; API path refused (403) and the administrator's account cannot open repository settings (07-09-2026) | Owner | when available, before deployment night | |
| Gate 4 (optional): delete the stale `.github/workflows/deploy.yml` from `master` | Owner | any time | |
| ~~Gates 9/10/17A/18A: `staging-rehearsal.sh dnds_rehearsal` on Lightsail~~ | Administrator | | 07-09-2026 — PASS (19A failed on Markdown parse) |
| ~~Gate 19A rerun after the plain-text fix~~ | Administrator | | 07-09-2026 — PASS (37/37 + 6/6; both alerts received in DN Accounts) |
| ~~Gate 23: run the local decode command (readiness §5.12) on your PC; report the three lines~~ | Administrator | | 07-09-2026 — PASS (legacy shape confirmed) |
...
| 07-09-2026 | Fresh gate 5 PASS at `c8d4c7b`, stamp `20260907-010839` (`GATE 5 REHEARSAL COMPLETE in 301s`). `scripts/auth/deployment-a-preflight.sh` added (read-only, no secrets; tested locally). `jwt-keys-setup.sh` now also writes `JWT_LEGACY_KID=legacy`; existing env files without it are fine (default). Exact rollback commands R1–R4 written into §5. | — | 07-09-2026 |
| 07-09-2026 | **Final pre-deployment review.** Gate 19A PASS on the Lightsail rerun (37/37 + 6/6, both alerts received). Gate 23 PASS from a real production token decoded locally. Ledger: 23 of 24 rows PASS; gate 6 PARTIAL (accepted); gate 4 PASS except Webhooks (admin access unavailable to both the administrator and this tool set). Verdict recorded in §2.1. No deploy, no merge, `dnds_prod` untouched. | — | 07-09-2026 |
| 07-09-2026 | Gate 4 webhooks: read-only hooks API tried for both repos with the available access → 403 (proxy policy); recorded as "PASS except Webhooks verification pending repository-admin access". | — | |
| 07-09-2026 | Lightsail staging rehearsal at `da4445e`: gates 9, 10, 17A, 18A PASS, forced-change 6/6; **19A FAIL** (`can't parse entities`). Fix: `services/telegram.js` honours `parseMode: null`; the three break-glass alerts (login, failed attempt, rotation-due cron) are plain text with one-line dynamic fields; regression test added; harness now also asserts no parse-mode rejection. No deploy; `dnds_prod` untouched; `dnds_rehearsal` kept. | 1, 2, 7, 11, 20 re-run → PASS (588 tests); 19A rerun pending | 07-09-2026 |
