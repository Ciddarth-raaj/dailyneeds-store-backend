...
```bash
~/mysql84/bin/mysql --defaults-extra-file=~/.stage0a/app.cnf -e 'DROP DATABASE `dnds_rehearsal`'   # the app identity created it (06-09-2026 run) and can drop it; decision 07-09-2026: KEEP until deployment night, the fresh gate 5 run replaces it
rm -f ~/.stage0a/admin.cnf                                                                     # only if you ever created one (the 06-09-2026 run did not); admin credential gone
